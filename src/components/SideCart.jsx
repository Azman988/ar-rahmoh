...
export function SideCart({ close }) {
    ...
    const handleWhatsAppCheckout = () => {
        ...
        let message = `*AR-RAHMOH ISLAMIC CURE - NEW INCOMING ORDER*%0A%0A`;
        let totalSum = 0;

        cart.forEach((item, index) => {
            const itemCost = item.price * item.qty;
            totalSum += itemCost;
            message += `${index + 1}. *${item.name}*%0A   Quantity: ${item.qty} units%0A   Price: ₦${item.price.toLocaleString()} each%0A   Subtotal: ₦${itemCost.toLocaleString()}%0A%0A`;
        });

        message += `*TOTAL ORDER VALUE:* ₦${totalSum.toLocaleString()}%0A%0A_Please verify product stock lines and transmit invoice billing statements to proceed._`;
            ...
            <div className="sidebar-drawer-card bg-white position-fixed top-0 end-0 bottom-0 shadow-2xl border-start border-light-dark d-flex flex-column justify-content-between animate-slide-left" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex-grow-1 overflow-y-auto p-3 d-flex flex-column gap-3">
                    {cart.length === 0 ? (
                        ...
                        cart.map(item => (
                            <div key={item._id} className="p-3 rounded-3 border border-light-dark bg-light-subtle d-flex align-items-center gap-3 position-relative group-item">
                                ...
                                <div className="flex-grow-1 min-w-0">
                                    <h6 className="fw-bold text-dark-emerald fs-7 mb-1 text-truncate w-100">{item.name}</h6>
                                    <p className="fw-bold text-success fs-7 mb-2">₦{(item.price * item.qty).toLocaleString()}</p>

                                    <div className="d-flex align-items-center gap-2">
                                        <button onClick={() => updateCartQty(item.id, 'dec')} className="btn btn-white border p-1 rounded-2 d-inline-flex align-items-center justify-content-center qty-btn-mini">
                                            <Minus size={12} />
                                        </button>
                                        <span className="fw-bold px-1.5 text-dark fs-7 select-none mini-counter-width-sm text-center">{item.qty}</span>
                                        <button onClick={() => updateCartQty(item.id, 'inc')} className="btn btn-white border p-1 rounded-2 d-inline-flex align-items-center justify-content-center qty-btn-mini">
                                            <Plus size={12} />
                                        </button>
                                    </div>
                                </div>
                                <button onClick={() => removeFromCart(item.id)} className="btn btn-link text-danger p-1.5 position-absolute top-3 end-3 rounded-3 hover-bg-danger transition-all remove-item-btn" aria-label="Delete Item">
                                    <Trash2 size={15} />
                                </button>
                            </div>
                        ))
